...
| 技能 | 当前定位 | 后续设计需求 |
| --- | --- | --- |
| 水龙破 | 前方飞行水龙，路径伤害 | 进入 `SKILL_DEFS`，支持升级奖励读取伤害倍率；大招期间仍保持直线处理 |
| 打潮刃 | 前方水流剑气，短距离高覆盖 | 进入 `SKILL_DEFS`，明确命中框和 UI 说明；大招期间仍保持贴身覆盖定位 |
| 静水返 | 防护水幕，受击反击 | 明确抵挡次数、反击目标、与 Boss 技能交互；大招期间仍保持防反容错定位 |
| 终式·月潮无间 | 限时强化状态，提升移动、跳跃、普攻节奏和伤害；普攻生成短促残影水刃 | 替换旧的瞬时范围爆发定位；读取 `ultimateLevel` 决定持续时间、移动、跳跃、攻速、伤害和残影追加伤害 |

普通技能池后续可以继续扩展直线、扇形、防反、位移、控场和破甲方向，但不要因为大招重做而把所有技能同质化。先把现有三招配置化，避免 XP、装备和大招等级接入时把倍率写散。
