...
> 实现状态：目标设计，部分源码已接入。当前源码已有 `bossKills`、Boss 注册表和终幕 Boss 雏形，但敌人 director、按幕敌人池、完整 6 基础 Boss + 6 蚀醒 Boss 阶梯仍未完成。其余设计文档的幕数、Boss 解锁、敌人解锁和威胁值都以本表为准。
...
| 7 | 蛛弦·蚀醒 | 加强 | + 全屏蛛网收束「千丝牢笼」 | 常规池 8，觉醒 profile shuffle cycle | 90-120s | 35-50% |
