...
| 技能与大招 | 三个技能、大招、技能能量、大招能量已实现 | 建立 `SKILL_DEFS` 注册表和解锁/升级读取方式 | 部分实现 | P3 | `src/constants/assets.ts`、`src/entities/player.ts` |
...
| 幕数与威胁值 | 当前难度主要读取 `elapsed`，没有幕数和统一威胁值 | `act = bossKills + 1`，`threatScalar` 协调敌人、Boss、平台和奖励 | 目标设计，未实现 | P1 | `docs/numeric-system/act-and-threat.md` |
