...
> 实现状态：目标设计，未实现。本文定义高台宝藏的生成节奏、路线约束、动态奖励、三选一、队列边界和验收口径；不表示当前源码已经接入该系统。
...
当前源码没有高台宝藏。旧版平台攻击/生命水晶、攻击/治疗宝箱和 `rewardDebt` 已移除；现有 `riskFork` 只是“安全主路 + 高风险支路”的地图片段，不提供奖励。
...
- 超过等待上限仍没有合格平台时，请求地图生成一次符合约束的 `riskFork` 变体：安全主路加高台奖励支路。
- 玩家主动错过后，本幕不补刷。
...
第一版排除 `breather`、`safeBridge`、`stairDown` 和 `hoverPair`。如果原始 `riskFork` 风险台过窄或悬浮，宝藏变体必须将其调整为静止且足够宽的平台，而不是把不公平路线直接标成奖励路线。
