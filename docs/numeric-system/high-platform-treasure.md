...
实现按三层拆分：`highPlatformTreasure.ts` 管理每幕机会、附着、解封、驻留领取和开匣定格；`treasureRewards.ts` 生成并结算不可变奖励快照；`highPlatformTreasure.ts` 实体绘制模块负责一次性右缘月潮光晕、光柱、平台辉光、四帧灵匣和领取反馈。领取后的短开匣定格继续推进自身视觉计时但冻结玩法，最终打开帧留在 Canvas 上，再由独立的宝藏 overlay 接管选择。
...
- 承载台接近屏幕右侧且尚未入屏时，在与宿主高度对齐的右缘短促闪现一次月潮光晕并自然散逸；不绘制箭头、文字或常驻屏幕标记。
- 进入屏幕后显示约 `80px` 高的弱光柱和平台边缘辉光。
- 光晕与每个宝藏唯一的一次提示音由同一接近边沿触发。
...
具体的“前奏最后窗口”应由 director 暴露语义状态，不要在宝藏模块复制 Boss 计时公式。首轮目标是从最后可附着时点到不可领取时点至少给玩家约 `10s` 的完整攀登窗口；一次性光晕本身不持续占据这段时间。
...
- 承载平台层级、片段类型、是否为强制支路。
- 预告是否触发、宿主是否入屏、玩家是否开始登高、是否领取、是否错过。
...
- 玩家在需要决定路线前能够感知一次无符号右缘光晕，入屏后由世界内光柱和平台辉光继续指引。
...
- 每幕宝藏机会成功附着率 `>= 98%`。
- Boss 重叠率 `< 1%`。
- 预告触发后尝试进入 `high / top` 路线的比例提升至少 `20` 个百分点。
- 熟悉系统后的领取率保持在 `45-70%`。
