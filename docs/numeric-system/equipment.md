...
> 实现状态：第一版已接入。当前源码实现了 18 件装备、Boss 三选一、月潮灵匣低频装备机会、三槽位换装和暂停页展示；精良/觉醒品质由当前幕段决定，同 ID 升品仍只来自 Boss 奖励。

## Purpose

定义第一版装备系统的完整设计：装备不是静态属性堆叠，而是单局内的构筑规则。玩家主要通过 Boss 掉落三选一获得装备，也可从月潮灵匣得到低频的明确装备机会；装备会在 `blade`、`garb`、`talisman` 三个槽位中替换构筑方向，让同一套基础动作在不同局内形成不同打法。
...
4. 装备按流派标签组织，同流派提供轻量共鸣；流水系在 2 / 3 件时获得持续恢复。
5. 品质提升主要改变机制完整度，数值只是辅助。
6. 装备是单局内奖励，死亡、重开或返回开始界面后清空。
7. 第一版不做局外养成、强化、随机词缀、局外套装收集和正式图标依赖。
...
Boss 装备选择和经验升级选择必须使用不同队列，不能互相覆盖。如果同一帧同时触发多个选择，优先级建议为：Boss 装备选择 > 经验升级选择。残灵拾取是无 overlay 的资源收集，不进入该选择队列。

高台秘藏使用第三个独立队列，顺序为 Boss 装备选择 > 高台秘藏选择 > 经验升级选择。它只提供明确的一件新装备或同槽替换，不提供同 ID 品质提升，也不会再打开第二层装备三选一；完整边界见 [high-platform-treasure.md](high-platform-treasure.md)。
...
## Primary Stat Percentages

每件装备在保留原有机制的同时，按当前品质百分比提高槽位主属性。百分比作用于当前等级的裸属性，不包含 `attackBonus` 等独立额外加成；最终属性统一四舍五入为整数。同一装备只使用当前品质的比例，不叠加低品质比例。残灵不提供攻击属性，因此也不参与该派生。

| 装备 ID | 主属性 | 普通 | 精良 | 觉醒 |
| --- | --- | ---: | ---: | ---: |
| `flow_blade` | 攻击力 | `+12%` | `+25%` | `+38%` |
| `burst_blade` | 攻击力 | `+14%` | `+27%` | `+40%` |
| `shadowstep_blade` | 攻击力 | `+11%` | `+24%` | `+37%` |
| `hunt_blade` | 攻击力 | `+13%` | `+26%` | `+39%` |
| `risk_blade` | 攻击力 | `+15%` | `+28%` | `+41%` |
| `tempo_blade` | 攻击力 | `+10%` | `+23%` | `+36%` |
| `flow_garb` | 最大生命 | `+10%` | `+20%` | `+30%` |
| `burst_garb` | 最大生命 | `+12%` | `+22%` | `+32%` |
| `shadowstep_garb` | 最大生命 | `+9%` | `+19%` | `+29%` |
| `hunt_garb` | 最大生命 | `+11%` | `+21%` | `+31%` |
| `risk_garb` | 最大生命 | `+13%` | `+23%` | `+33%` |
| `tempo_garb` | 最大生命 | `+8%` | `+18%` | `+28%` |
| `flow_talisman` | 技能能量上限 | `+11%` | `+22%` | `+35%` |
| `burst_talisman` | 技能能量上限 | `+10%` | `+21%` | `+34%` |
| `shadowstep_talisman` | 技能能量上限 | `+12%` | `+23%` | `+36%` |
| `hunt_talisman` | 技能能量上限 | `+13%` | `+24%` | `+37%` |
| `risk_talisman` | 技能能量上限 | `+14%` | `+25%` | `+38%` |
| `tempo_talisman` | 技能能量上限 | `+9%` | `+20%` | `+33%` |

等级或装备变化时，都先计算当前等级的基础攻击、最大生命和技能能量上限，再乘当前装备比例；不能在已经包含装备加成的旧值上继续相乘。换装、升品或降品后立即重算，当前生命和技能能量分别夹取到新的上限。死亡、重开或返回开始界面清空装备时，这些加成也一并清除。
...
- 除装备主属性百分比外，品质提升以机制完整度为主，数值只作为门槛、冷却、稳定性的辅助调参。
...
### High-Platform Treasure Boundary

> 实现状态：已实现。

高台秘藏可以提供当前幕段品质的具体装备，但必须保持以下边界：

- 只提供玩家尚未拥有的装备 ID。
- 可以填补空槽或替换同槽装备。
- 不提供同 ID `tierUpgrade`，装备品质提升继续只通过 Boss 奖励进入。
- 卡面直接显示装备、品质、效果和被替换装备；选择后直接入库并自动装备。
- 宝藏装备选择清理自己的队列，不能覆盖 Boss 装备或经验升级选择。
- 选择后与 Boss 装备共用换装结算：清空该槽位运行时状态并重算派生属性。

这让宝藏提供低频构筑机会，但不会取代 Boss 稳定的装备三选一和升品身份。动态出现权重与三选一构成见 [high-platform-treasure.md](high-platform-treasure.md)。
...
同流派共鸣保持轻量，目的是鼓励构筑路线而不是强迫凑套。实际效果按流派定义，未定义的档位不会获得隐含的通用效果。
...
| 流派 | 2 件效果 | 3 件增量效果 |
| --- | --- | --- |
| 流水 | 流水刃与潮声符触发门槛降低 `1`；每秒恢复 `2` 点技能能量 | 流水核心效果触发时额外获得 `2` 点技能能量；每秒恢复 `1` 点生命 |
| 破势 | 破月刃 Boss 斩杀线从 `35%` 提高至 `50%`；燃魂符冷却从 `90` 帧降至 `60` 帧 | 燃魂符核心触发时额外获得 `2` 点技能能量；精良或觉醒的技能命中额外触发也会再获得 `2` 点 |
| 影步 | 获得 `15%` 闪避几率；闪避时本次伤害、受击反馈、击退和致死保护均不触发 | 触发「掠影」时额外获得 `10` 点技能能量 |
| 狩猎 | 狩牙刃门槛 `2→1`；逐猎衣门槛 `3→2`；连珠符冷却 `4→3` 秒 | 狩牙刃常态生效；逐猎衣门槛 `3→1`；连珠符冷却 `4→2` 秒 |
| 残心 | 低血时攻击速度、移动速度提高 `15%` | 双速从 `15%` 提高至 `30%`；获得抵挡 `1` 次伤害的护盾，冷却 `5` 秒 |
| 节奏 | 暂无套装效果 | 暂无套装效果 |

3 件效果均保留已定义的 2 件机制；残心双速使用完整的 `30%`，不会与 2 件套的 `15%` 叠加。
...
```ts
baseAttack = Math.round(baseAttackForLevel(runLevel) * (1 + bladePrimaryStatRatio));
maxHp = Math.round(maxHpForLevel(runLevel) * (1 + garbPrimaryStatRatio));
skillEnergyMax = Math.round(maxSkillEnergyForLevel(runLevel) * (1 + talismanPrimaryStatRatio));
hp = min(hp, maxHp);
skillEnergy = min(skillEnergy, skillEnergyMax);
```

`baseAttack`、`maxHp` 和 `skillEnergyMax` 物化保存“当前等级基础值按当前装备比例派生后的整数”；每次等级或装备变化都从等级公式重新计算，不在旧值上重复相乘。
...
| 项 | 设计边界 |
| --- | --- |
| 装备直接攻击 | 刃器按装备和品质提供 `+10%` 至 `+41%`，主要差异仍来自专属机制 |
| 装备直接最大生命 | 衣装按装备和品质提供 `+8%` 至 `+33%`，机制仍不能让生存流无脑站桩 |
| 装备直接技能能量上限 | 饰符按装备和品质提供 `+9%` 至 `+38%`，不直接补充当前技能能量 |
...
暂停页装备详情和 Boss 装备奖励卡必须同时展示该流派的 2 件与 3 件套装效果。尚未定义效果的档位统一显示“暂无套装效果”，避免玩家把空白误认为 UI 遗漏。
...
- 换装、升品或降品后从等级裸属性重新计算装备主属性；最大生命降低时将当前生命夹取到新上限，技能能量上限降低时将当前技能能量夹取到新上限。
...
- 每个流派都有输出、生存、资源三件装备。
- 每件装备有普通、精良、觉醒三档机制描述。
- 每件装备按普通、精良、觉醒提供独立的主属性百分比，同槽装备存在小幅差异。
- 装备百分比只作用于当前等级裸属性，结果取整且不包含 `attackBonus` 等独立加成。
- 装备主属性随等级或装备变化从基线重算，死亡、重开或返回开始界面后随装备清除。
...
- 影步系 2 件提供 `15%` 闪避，3 件在触发「掠影」时额外获得 `10` 点技能能量。
- 掉落三选一能表达“强化已有路线 / 补足短板 / 改变打法”。
- 同流派有轻量共鸣；流水系 2 / 3 件持续恢复按实际装备数量即时生效。
- 所有装备都能在无正式图标的情况下用 UI 文案表达；暂停装备详情与 Boss 奖励卡都显示对应流派的 2 / 3 件效果。
