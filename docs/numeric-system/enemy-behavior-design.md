...
| 攻击窗口 | 距离玩家一定范围时立即锁定朝向；前摇序列每约 `0.117` 秒推进一格，末帧停留 `1` 秒；随后普通形态冲刺 `1.33-1.78` 个视觉身位，觉醒/终幕冲刺 `4-4.67` 个视觉身位 |
...
| 解锁规则 | `tier 2`；第 2-3 幕可进入解锁顺序 |
| 主要压力 | 平台落点和垂直空间 |
| 状态机 | 普通/觉醒：`stalk -> windup -> leap -> impact -> recover`；终幕：`stalk -> windup -> skyRise -> skyWait -> skyFall -> impact -> recover` |
| 攻击窗口 | 进入范围后停顿 `14-24` 帧，锁定玩家当前位置及其所在平台；普通/觉醒抛物线跳跃，终幕离屏后沿预警纵线重砸 |
| 恢复窗口 | 落地后 `20-30` 帧拔出脚爪或起身 |
| 玩家反制 | 看前摇离开落点、下落攻击反打、潮龙·破阵在起跳前击杀 |
| 不可做边界 | 落点必须贴合平台/地面；锁点后不追踪玩家，终幕坠落前必须完整显示红线预警 |
...
| 阶段 | 跳跃变化 | 追加威胁 | 落地反馈 |
| --- | --- | --- | --- |
| 普通 | 可跳到玩家所在平台 | 无 | 基础冲击伤害与红褐落点圈 |
| 觉醒 | 缩短前摇与滞空时间 | 跳跃约三成进度时从背部一次发射 `8` 向骨刺 | 扩大冲击范围 |
| 终幕 | 上升出屏，离屏等待 `42` 帧后垂直坠落 | 两条红线标记真实重击宽度；锁点后不随玩家移动 | 高额单次伤害与 `16` 枚旋转飞石 |
...
| --- | --- | --- | --- |
| 普通 | 无火球 | 无 | 只吸收正面伤害 |
| 觉醒 | `1` 枚火球 | 前方约 `230px`，滚速 `3.4` | 只吸收正面伤害 |
| 终幕 | `2` 枚火球 | 近 `150px / 2.8`，远 `340px / 4.2` | 正面实际盾损反弹 `25%`，单次最多 `12`；背击、破甲、破盾和非玩家伤害不反弹 |

火球按 `launch -> roll -> explode` 推进，最多同时存在 `4` 枚。滚动不直接扣血，爆炸第三帧单次结算；终幕两球使用不同出膛和滚动速度，避免视觉重叠。
...
| 攻击窗口 | 低空悬停后收翼前摇，沿斜线俯冲掠过玩家附近；觉醒后拍翼可发出单枚横向音波刃 |
| 恢复窗口 | 俯冲后重新展开翼膜上抬，`18-28` 帧可被反击 |
| 玩家反制 | 跳跃普攻、潮龙·破阵、提前离开俯冲线 |
| 不可做边界 | 不能高到玩家无法命中；音波刃不能追踪、连发或形成弹幕，否则和 `caster` 混淆 |
| 同屏限制 | 最多 `2`，同时俯冲最多 `1` |
...
| 攻击窗口 | 直接攻击弱；核心威胁是分阶段光环：普通半径 `300px`，敌人获得 `+15%` 移速与攻击力；觉醒半径 `600px`，敌人获得 `+30%` 移速与攻击力；最终阶段全战场生效，敌人免伤并获得 `+50%` 移速与攻击力 |
| 恢复窗口 | 被击中时光环短暂抖动或失效 `10-16` 帧 |
| 玩家反制 | 潮龙·破阵远程点杀、绕过前排、用大招清掉整波 |
| 不可做边界 | 光环不影响 Boss；最终阶段免伤只作用于敌人，不作用于 Boss |
| 同屏限制 | 同场最多 `1` 个 |
