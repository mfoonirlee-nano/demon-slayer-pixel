...
- 现有运行时已通过 `sheetIndex -> archetype` 映射接入 `chaser`、`crawler`、`runner`、`duelist`、`brute`、`caster`、`glider`、`binder`；后续需要整理为显式配置表。
...
- 接入 XP 和升级三选一。
- 升级奖励接入普通技能等级，已解锁技能每局初始 Lv1，单技能上限 Lv3，死亡或重开清空回 Lv1。
...
- 技能释放、HUD、伤害公式改为读取技能定义和 `skillLevels`。
- 当前三招作为默认解锁技能。
- 后续新增技能资源只新增配置和资源元数据。
