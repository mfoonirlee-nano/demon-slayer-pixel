...
> 实现状态：源码已接入 `act`/`actBand` helpers、威胁值、Boss 出场门槛、平台分段和敌人复杂度分层；数值仍需试玩调参。

## Purpose

说明数值系统的目标边界：用 Boss 击杀驱动幕数，用统一威胁值协调敌人、Boss 和平台，并通过敌人组成间接调节残灵总供给。各系统通过注册表支持内容扩展。
...
| --- | --- | --- | --- | --- | --- | --- |
| intro | 1-6 | 逐幕熟悉一种新压力，6 幕内见齐 12 敌人 | 解锁数量 `3,5,7,9,11,12`；常规池 `3,4,5,6,7,8`；1-3 固定教学 profile，4-6 随机补缺失机制 | 从安全片段逐步加入阶梯/悬浮/高风险支路 | 6 个基础 Boss，各承担一种机制 | 普通敌人掉落残灵，每类敌人按复杂度给固定数量 |
| awakened | 7-12 | 蚀醒难度墙，基础 Boss 蚀醒形态 1:1 重现 | 不引入新敌人；常规池 8；6 个 awakened profile 每局 shuffle 且不重复 | 高风险片段权重更高，仍保喘息 | 6 个蚀醒 Boss：+ 新月蚀术招 + 多一阶段 | 装备品质带提升，残灵单个掉落值不随幕数额外放大 |
...
| `riskFork` | `0.2` | `0.7` | `1.3` | `1.5` | `0.6` |

残灵供给不使用幕段倍率：分裂体 `1`、T1 `3`、T2 `4`、T3/T4 `5`、elite `8`。因此后期供给只会随敌人组成与击杀效率间接改变，不会因幕数对同一敌人额外放大。储存上限、消耗与治疗规则见 [rewards.md](rewards.md)。
...
| `ACT_CONFIGS` | 每幕敌人池、Boss 池、平台权重和目标节奏 |
