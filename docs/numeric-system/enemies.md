...
- `brute`：站立持盾推进，非大招伤害先扣盾牌耐久，破盾后进入硬直并改用无盾推进/横扫；大招绕过盾牌并在存活时同步破盾。
- `caster`：保持施法距离，前摇后发射追踪幽火。
...
| `brute` | `3.25` | 同场最多 `2`，同时攻击最多 `1` | 盾牌耐久为本体生命 `40%`；完整盾牌先吃非大招伤害，破盾后改用无盾横扫 |
| `caster` | `1` | 每个 caster 幽火最多 `2` | 保持距离，前摇后发射追踪幽火 |
