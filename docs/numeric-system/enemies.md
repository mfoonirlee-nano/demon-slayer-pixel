...
- `brute`：站立持盾推进，正面伤害由盾牌完全吸收且不溢出，破盾后进入硬直并改用无盾推进/横扫；装备 Lv3 `armor_break` 后，玩家其他攻击会将正面伤害按本体/盾牌各 50% 拆分，`armor_break` 本身仍直接破盾但不打本体。觉醒盾击追加单枚地滚火球，终幕追加一近一远两枚高伤爆炸火球，且终幕举盾会反弹部分正面盾伤。
- `caster`：保持施法距离，前摇后发射追踪幽火；普通状态每 `5s` 单发，觉醒每 `3s` 三连发，终幕每 `3s` 六连发并加快幽火序列帧。
- `leaper`：低速跟踪，锁定玩家位置及其所在平台后跳跃落地；觉醒追加八向背刺，终幕改为带红线预警的离屏坠落；当前按 `elapsed >= 35s` 才进入随机候选。
...
| `brute` | `3.25` | 同场最多 `2`，同时攻击最多 `1`，火球最多 `4` | 盾牌耐久为本体生命 `200%`；Lv3 `armor_break` 装备被动提供 `50%` 穿盾；觉醒/终幕盾击追加 `1/2` 枚固定落点地滚爆球；终幕 `guard` 反弹 `25%` 实际正面盾伤，单次上限 `12`；`armor_break` 直接破盾，破盾后改用无盾横扫 |
| `caster` | `1` | 每个 caster 幽火最多普通 `3` / 觉醒 `15` / 终幕 `30` | 保持距离，前摇后发射追踪幽火；普通 `1` 发，觉醒 `3` 发紫红幽火，终幕 `6` 发暗红幽火且动画更快 |
| `leaper` | `1.05` | `elapsed >= 35s` 后随机生成，同场最多 `2`，同时锁定落点最多 `1` | 三阶段均可跳到玩家平台；觉醒空中释放八向背刺，终幕离屏预警后重砸，落地后硬直 |
