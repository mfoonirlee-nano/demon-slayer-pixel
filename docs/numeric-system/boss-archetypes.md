...
终幕万相血月不走这条曲线，用独立 `baseHp` 定位到普通 Boss 的 `2.2x-2.8x`（见本文「终幕 Boss：万相血月」）。蚀醒幕（`act >= 7`）的额外难度来自多一阶段 + 蚀醒招 + 强化召唤，而非额外血量手写值。
...
| --- | ---: | --- | --- |
| 蛛弦·蚀醒 | 7 | 千丝牢笼：预警线后逐段收束全屏蛛网，分段封锁地面/空中 | 看预警线提前走位到安全段 |
| 雾骨·蚀醒 | 8 | 浓雾葬：短时遮蔽视野 + 多点延迟骨刺连爆 | 记预警点，雾中靠落点记忆 |
| 镜魇·蚀醒 | 9 | 真影错位：每阶段开始真假身随机互换 + 分身同步反射上一个技能 | 阶段切换瞬间重新辨真身，谨慎丢技能 |
| 牙岚·蚀醒 | 10 | 岚牙连闪：三段跨场冲刺连段 | 连续读三次起手方向，找跳/反击窗口 |
| 灯烬·蚀醒 | 11 | 燎原灯阵：火线连成移动网格 + 灰烬减速区叠加 | 走网格缝隙，避免被灰烬粘住 |
| 枯铃·蚀醒 | 12 | 双调枯铃：双频错相声波环 + 强制停拍反震 | 停拍窗口内停手，否则被反震 |
