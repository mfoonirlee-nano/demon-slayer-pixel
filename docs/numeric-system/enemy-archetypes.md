...
| 攻击窗口 | 距离玩家一定范围时停顿 `14-18` 帧，然后冲刺 `28-35` 帧 |
...
| 主要压力 | 持续占位和目标优先级 |
| 状态机 | `advance -> brace -> stomp -> recover -> advance` |
| 攻击窗口 | 偶尔重踏或短霸体推进，伤害高但前摇长 |
| 恢复窗口 | 重踏后 `24-34` 帧停顿，霸体结束 |
| 玩家反制 | 贰之型、下落攻击、大招 |
| 不可做边界 | 霸体不能免疫大招；不能高速追击玩家 |
| 同屏限制 | 第 3 幕初次出现最多 `1`，后续最多 `2` |
| 可读性约束 | 靠体量和甲壳读出高血重型定位 |
