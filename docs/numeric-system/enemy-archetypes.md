...
```ts
enemyHp = round(
  (baseHp + k * hpPerBossKill)
  * threatScalarForRun(k, elapsedSeconds)
  * actHpScale(k)
)
enemyDamage = min(
  damageCap * actDamageCapScale(k),
  (baseDamage + k * damagePerBossKill) * actDamageScale(k)
)
enemySpeed = (
  baseSpeed + k * speedPerBossKill + random(0, randomSpeed)
) * actSpeedScale(k)
```
...
表格是当前运行时配置基准，仍需用试玩数据微调。HP 会进入 `threatScalar` 与幕数倍率计算；伤害保留随幕放大的上限，避免长时间局内成长把普通敌人接触伤害推到不可解。

| Archetype | HP | 伤害 | 速度 | 预算 | 击杀分 | 技能能量 | 大招能量 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `chaser` | `32 + k*0.70` | `8 + k*0.35`，cap `16` | `2.86 + k*0.12 + rand(0.25)` | `1` | `10` | `10` | `2.7` |
| `crawler` | `16 + k*0.90` | `6 + k*0.30`，cap `14` | `1.05 + k*0.06 + rand(0.95)` | `1` | `9` | `8` | `2.0` |
| `runner` | `24 + k*0.50` | `9 + k*0.45`，cap `18` | `1.25 + k*0.08 + rand(0.85)` | `1.2` | `11` | `9` | `2.4` |
| `duelist` | `48 + k*0.45` | `8 + k*0.55`，cap `20` | `0.84 + k*0.05 + rand(0.72)` | `1.5` | `14` | `12` | `3.2` |
| `leaper` | `40 + k*0.40` | `8 + k*0.50`，cap `20` | `0.72 + k*0.04 + rand(0.50)` | `1.6` | `15` | `12` | `3.2` |
| `brute` | `80 + k*3.70` | `10 + k*0.80`，cap `24` | `0.42 + k*0.03 + rand(0.45)` | `2.2` | `22` | `16` | `4.5` |
| `caster` | `40 + k*0.55` | `6 + k*0.45`，cap `16` | `0.48 + k*0.03 + rand(0.42)` | `1.6` | `18` | `14` | `4.0` |
| `glider` | `32 + k*0.55` | `7 + k*0.40`，cap `18` | `0.92 + k*0.05 + rand(0.45)` | `1.7` | `16` | `12` | `3.4` |
| `burrower` | `40 + k*0.00` | `9 + k*0.50`，cap `22` | `0.58 + k*0.04 + rand(0.45)` | `2.2` | `16` | `12` | `3.4` |
| `splitter` | `64 + k*0.20` | `6 + k*0.35`，cap `16` | `0.62 + k*0.04 + rand(0.50)` | `2` | `16` | `12` | `3.0` |
| `binder` | `40 + k*0.20` | `6 + k*0.25`，cap `14` | `0.42 + k*0.02 + rand(0.35)` | `2.6` | `18` | `14` | `4.0` |
| `warden` | `64 + k*0.00` | `4 + k*0.20`，cap `10` | `0.38 + k*0.02 + rand(0.30)` | `2.8` | `24` | `18` | `5.0` |

这组生命值把原先偏后期的 `hpPerBossKill` 前移到 `hpBase`：一级普攻伤害为 `16` 时，开局形成 `1 / 2 / 3 / 4 / 5` 击耐久层；第 13 幕本体生命与调整前相比仅约 `-0.7%` 到 `+3.3%`，不会额外制造后期血墙。`brute` 的正面盾牌耐久另算。

招式伤害例外：

- `runner` 的 `9` 点接触伤害只在实际移动的冲刺帧生效；蓄力、落地和空中停帧不造成接触伤害。
- `crawler` 开局前扑为 `6 * 1.45 + 1 = 9.7` 点伤害。
- `caster` 幽火独立使用 `min(14, 7 + elapsedMinutes * 0.45)`，觉醒/终幕再乘 `1.2 / 1.5`。
- 蚀醒 `glider` 幽刃为当前基础伤害的 `1.25` 倍，约占同阶段无装备最大生命的 `8%` 到 `10%`。
- `binder` 的符纸持续伤害维持现值；它绕过装备减伤且已经附带控制，不随本轮基础伤害一起上调。
...
普通敌人可优先在右侧屏幕外平台出生。第 1-3 幕分别以 `1.5% / 3% / 4.5%` 的概率尝试近景遮挡出生；第 4 幕起按每幕 `4.5%` 递增，最高 `45%`。除 `glider` 这类空中入场敌人外，从地面状态初始化的常规敌人都可使用当前可见遮挡物；候选物件的绘制矩形必须大于敌人的完整基础视觉尺寸、中心点落在屏幕内，而且整个物件不能覆盖玩家，非匀速移动的 Boss 前奏鸟居不参与。命中后同一个敌人绘制于整层近景下方，暂停常规攻击 AI 和接触伤害，并以最低 `2.4px/帧` 沿玩家方向移出；当完整视觉边界与按近景速度滚动后的所选物件完全分离时，才提升到普通敌人图层。未命中规则时仍回退到原有屏幕边缘或平台出生。Boss 召唤和 debug spawn 不使用近景遮挡物出生。
...
Boss 出现由“完成若干常规波 + 最短时间”触发，并有最长时间兜底。当前运行时把原设计秒数按 `0.75` 压缩，但保留最低波次数不变，让同样波次量更快完成：
