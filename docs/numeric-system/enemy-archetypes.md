...
普通敌人可优先在右侧屏幕外平台出生。第 1-3 幕分别以 `1.5% / 3% / 4.5%` 的概率尝试近景遮挡出生；第 4 幕起按每幕 `4.5%` 递增，最高 `45%`。除 `glider` 这类空中入场敌人外，从地面状态初始化的常规敌人都可使用当前可见遮挡物；候选物件的绘制矩形必须大于敌人的完整基础视觉尺寸、中心点落在屏幕内，而且整个物件不能覆盖玩家。命中后只在敌人的视觉边界内裁剪重绘遮挡物，并在 `36` 帧内按首次实际移动方向逐步揭示；暂时静止的后排敌人使用朝向玩家的回退方向。未命中规则时仍回退到原有屏幕边缘或平台出生。Boss 召唤和 debug spawn 不使用近景遮挡物出生。
