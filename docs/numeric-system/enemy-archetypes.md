...
> 实现状态：目标设计，未实现。当前源码仍是单一敌人数值与 AI；现有 6 张正式敌人素材可逐步映射到不同 archetype。
...
| --- | --- | --- | --- | ---: | --- |
| `chaser` | `enemy_1.png` | 基础追击 | 从屏幕一侧横穿到另一侧，未击杀则折返重复 | `1` | 基础核心，后期降权 |
| `crawler` | `enemy_2.png` | 低矮穿插 | 低血贴地追踪，压缩地面空间 | `1` | 前期核心，后期轮换 |
| `runner` | `enemy_3.png` | 快攻压力 | 短前摇后冲刺，迫使跳跃或远程处理 | `1` | 前中期核心，后期轮换 |
| `duelist` | `enemy_5.png` | 近战精英 | 接近后短距离斩击 | `2` | 中期近战压力 |
| `leaper` | 待制作 | 跳跃突袭 | 蓄力后抛物线跳向玩家落点 | `2` | 平台和落点压力 |
| `brute` | `enemy_6.png` | 高血目标 | 慢速高血，短霸体窗压迫空间 | `3` | 中期阻挡压力 |
| `caster` | `enemy_4.png` | 远程施压 | 保持距离，周期性发射投射物或召唤标记 | `3` | 中后期远程压力 |
...
| 当前素材 | 视觉特征 | 当前问题 | 目标改造 |
| --- | --- | --- | --- |
| `enemy_1.png` | 裸身奔跑鬼 | 和所有敌人同数值 | 作为默认 `chaser`，承担横穿追击的基础怪规则 |
...
| 主要压力 | 地面空间和跳跃落点 |
| 状态机 | `spawn -> move -> recoverOnHit/dead` |
| 攻击窗口 | 全程以低伤接触为主，不额外蓄力攻击 |
| 恢复窗口 | 被击退或撞到平台边缘时短暂停顿 |
| 玩家反制 | 下落攻击、壹之型直线清理、普攻 |
| 不可做边界 | 碰撞体不能低到普攻打不到；不能潜行，否则和 `burrower` 混淆 |
| 同屏限制 | 第 1 幕最多 `3`，后续最多 `5` |
| 可读性约束 | 低矮但可命中，贴地轮廓区别于站立敌人 |
