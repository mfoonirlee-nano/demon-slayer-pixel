...
| `enemies/binder/binder_zone.png` | `960x120` | 4 | `240x120` | `bindingZone` | 偏侧面视角的暗红/紫色地面咒圈；首帧出现，后 3 帧循环维持，约 `150` 帧内使玩家水平移动速度乘以 `0.45` |

Binder 运行时由 `BINDER_SHEETS` 和 `BINDER_ZONE_SHEET` 暴露并预加载。普通刷怪在 `elapsed >= 90s` 后才会抽取 binder；同屏最多 `1` 个 binder，主咒圈最多 `1` 个。咒圈不造成伤害，不禁用跳跃或攻击；玩家进入咒圈时会叠加偏红紫的减速滤镜和束缚线反馈。
