...
敌人图集按敌人拆分在 `assets/sprites/enemies/<enemy>/` 下；Boss 当前放在 `assets/sprites/enemies/boss/`。
...
| `enemies/binder/binder_zone.png` | `1920x120` | 8 | `240x120` | `bindingZone` | 完整暗红/紫色地面咒圈源图；外环、符文、中心脉冲和短光束逐帧变化，约 `150` 帧内使玩家水平移动速度乘以 `0.45` |
| `enemies/binder/binder_zone_back.png` | `1920x120` | 8 | `240x120` | `bindingZoneBack` | 咒圈上半/远端层，先于角色绘制 |
| `enemies/binder/binder_zone_front.png` | `1920x120` | 8 | `240x120` | `bindingZoneFront` | 咒圈下半/近端亮边层，在地面前景后绘制 |

Binder 运行时由 `BINDER_SHEETS`、`BINDER_ZONE_SHEET`、`BINDER_ZONE_BACK_SHEET` 和 `BINDER_ZONE_FRONT_SHEET` 暴露并预加载。普通刷怪在 `elapsed >= 90s` 后才会抽取 binder；同屏最多 `1` 个 binder，主咒圈最多 `1` 个。咒圈不造成伤害，不禁用跳跃或攻击；玩家进入咒圈时会叠加偏红紫的减速滤镜和束缚线反馈。
