...
雾骨额外使用独立的雾骨钉普攻、单点埋骨、成列压地和蚀醒合围施法图集；普攻在第 4 帧从手部高度斜向下释放独立的 4 帧骨钉飞行循环。三种地刺施法共享原有伤害与预警时序，但用不同本体轮廓提前区分单点、横排和由外向内合围。单点与成列招式的薄雾减速区、浓雾葬的局部遮蔽、蚀醒本体冷骨光与环身浓雾，以及击败时的骨架碎片和吹散雾团均由 Canvas 程序化绘制；这些表现不新增图集，也不改变现有 PNG 的切片契约。P3 成列施法结束后复用攻击动作进入锁向追击，P4 合围调用现有敌人素材生成一名蚀醒 `warden` 与一名基础召唤池同伴，同样无需新增雾骨专属图片。
