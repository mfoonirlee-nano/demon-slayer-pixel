...
`scenery/weather/` 下的四张落叶图集均为 `192x24` RGBA 横条，固定 `8` 帧、单帧 `24x24`：

| 文件 | 叶型 | 对应树变体 |
| --- | --- | --- |
| `falling-leaf-pine-tumble.png` | 3-5 针相连的松针枝束 | 基础树 `0/1/5/8/9/12/13/15/20/21/23`；全部高树 |
| `falling-leaf-willow-tumble.png` | 细长弯曲柳叶 | 基础树 `4/18` |
| `falling-leaf-broadleaf-tumble.png` | 短柄阔椭圆锯齿叶 | 基础树 `2/6/11/17/22` |
| `falling-leaf-bamboo-tumble.png` | 较短、硬直的披针竹叶 | 基础树 `16` |

基础树 `3/7/10/14/19` 是无叶枯木，不登记树冠来源。每张图的八帧都从正面、四分之三侧、薄边、背面推进到反向姿态并回到起始姿态；每帧可见 bbox 围绕 `(12, 12)`，四边至少保留 `3px` 全透明 gutter，薄边帧仍保留 `3-5px` 可见宽度。颜色使用低饱和铜褐、冷赭和暗酒红，配暗靛轮廓与极弱月蓝边光，避免与鲜红攻击预警或玩家的高亮月潮蓝混淆。视觉约束见 [`docs/art/environment.md`](art/environment.md)。

`FALLING_LEAF_SHEETS` 将四类图集登记为标准 `SpriteSheet` 并交由素材 manifest 逐张预加载。`REGULAR_TREE_FOLIAGE` 与 `TALL_TREE_FOLIAGE` 为有叶变体登记经过源图 alpha 与叶色复核的归一化叶簇锚点；远景叶只从高树树冠释放，近景叶只从基础树线释放，叶型直接继承来源树，不再单独随机。释放后叶片冻结当时的树冠原点并独立下落，不做屏幕横纵取模回卷；近景以原生 `24x24` 绘制，远景以整数半缩放 `12x12` 绘制。位置、阵风、透明度和选帧仍由确定性的场景时钟控制，因此暂停和战斗冻结不会让气候层偷跑。
...
`platform/acts/` 保存 13 张带 alpha 的 `544x128` PNG 图集，每幕一张。每张图固定包含三个切片：`chain=80x64 @ (0,0)`、`normal=160x96 @ (96,0)`、`wide=272x128 @ (272,0)`；`drawScale=0.75`，切片之间保留 `16px` 透明隔离区。每个切片的 `surfaceY` 分别标记经连续三行支撑和材质边缘共同校验的首个实体落脚带；草尖、蛛丝、骨刺等装饰可以高于该行，但不作为角色落脚线。四边继续保留透明 gutter，碰撞宽高仍由运行时平台状态决定。

| 幕 | Boss / 形态 | `surfaceY`（chain / normal / wide） | 平台视觉母题 | 文件 |
| ---: | --- | --- | --- | --- |
| 1 | 蛛弦 / 基础 | `12 / 15 / 11` | 蓝黑神社石、白蛛丝、暗红结扣、蛛足钩 | `act-01-spider-string.png` |
| 2 | 雾骨 / 基础 | `8 / 9 / 8` | 枯骨灰白、雾蓝冰晶、肋骨与脊椎 | `act-02-mist-bone.png` |
| 3 | 镜魇 / 基础 | `8 / 11 / 14` | 黑曜镜砖、冷蓝碎镜、银色裂边 | `act-03-mirror-dream.png` |
| 4 | 牙岚 / 基础 | `6 / 7 / 8` | 风蚀暗岩、象牙獠牙、爪痕与兽口 | `act-04-fang-gale.png` |
| 5 | 灯烬 / 基础 | `7 / 7 / 7` | 焦黑神社石、暗红熔火裂隙、灰烬 | `act-05-lantern-ember.png` |
| 6 | 枯铃 / 基础 | `9 / 11 / 20` | 暗金旧钟铜、裂铃、回纹声波 | `act-06-dead-bell.png` |
| 7 | 蛛弦 / 蚀醒 | `8 / 9 / 9` | 血红丝线核心、密网、渗血关节钩 | `act-07-spider-string-awakened.png` |
| 8 | 雾骨 / 蚀醒 | `6 / 9 / 8` | 密集骨林、强冷蓝鬼火、浓雾纹 | `act-08-mist-bone-awakened.png` |
| 9 | 镜魇 / 蚀醒 | `10 / 10 / 10` | 明暗双镜、高密镜裂、冷蓝晶簇 | `act-09-mirror-dream-awakened.png` |
| 10 | 牙岚 / 蚀醒 | `5 / 5 / 5` | 三重兽口节奏、血光尖牙、风刃痕 | `act-10-fang-gale-awakened.png` |
| 11 | 灯烬 / 蚀醒 | `5 / 5 / 5` | 多层灯牢、熔火网格、浓灰烬 | `act-11-lantern-ember-awakened.png` |
| 12 | 枯铃 / 蚀醒 | `7 / 8 / 9` | 双裂铃、暗金铜、双频错相回纹 | `act-12-dead-bell-awakened.png` |
| 13 | 万相血月 / 终幕 | `5 / 6 / 6` | 黑红王座石、血月纹、受控眷属残相 | `act-13-blood-moon-many-faces.png` |
...
`pickups/residual-spirit.png` 是一张 `96x96` 透明 PNG 单图，四边保留透明 gutter，运行时按 `24x24` 绘制。可见内容是单个蓝白月潮残灵光团，主体边缘保持清晰像素块；浮动、呼吸亮度和弱光晕由运行时程序叠加，不烘入大面积半透明烟雾。掉落物不做朝玩家的磁吸位移；接触拾取后本体消失，并由程序化蓝白光核和错峰尾粒沿短弧线飞入当前可见的 HUD 灵龛。

`pickups/moon-tide-treasure.png` 是一张 `512x128` RGBA 横向图集，固定 `4` 帧、单帧 `128x128`，运行时按 `84x84` 绘制并以平台表面为底部锚点。四帧依次是休眠闭匣、月印点亮、半启匣和完全开启；各帧可见内容的下边界统一为源图 `y=122`（exclusive，最后可见像素为 `y=121`），透明 gutter 不得裁掉屋檐、月冠或最终帧的三团灵火。该图集只承载灵匣本体，一次性右缘月潮光晕、弱光柱、平台边缘辉光、驻留进度环、飞散光点和开匣闪光由 Canvas 运行时绘制。
...
| `residual_spirit_heal_effect.png` | `1920x240` | 8 | `240x240` | 引灵治疗附着特效 | 纯 VFX；前 5 帧收拢两枚残灵，后 3 帧只在成功结算后播放峰值与散逸 |
...
`player/residual_spirit_heal_effect.png` 不包含角色或武器，也不替换当前玩家动作。运行时以 `154x154` 绘制在玩家中心和脚点上，源图脚环基线约为 `y=226`，因此使用 `anchorX=0.5`、`anchorY=0.942` 且不随朝向镜像。引灵进行中的 `0.6s` 只映射前 5 帧：两枚蓝白残灵从身体两侧向内旋入并压缩为月蓝核心；只有残灵真实扣除且生命恢复成功后，才在额外 `0.18s` 内播放第 6-8 帧的月白弯月、脚下潮环和泡沫散逸。失败复检、死亡和未引灵状态不绘制成功帧。各帧固定为 RGBA、四角全透明、可见内容不触边，且不参与碰撞或伤害。
...
| `boss/mist-bone/mist_bone_fog_veil.png` | `1536x160` RGBA | 6 | `256x160` | `MIST_BONE_FOG_VEIL_SHEET` |
| `boss/mist-bone/mist_bone_fog_roll.png` | `1536x160` RGBA | 6 | `256x160` | `MIST_BONE_FOG_ROLL_SHEET` |
| `boss/mist-bone/mist_bone_fog_wisp.png` | `1536x160` RGBA | 6 | `256x160` | `MIST_BONE_FOG_WISP_SHEET` |
...
| `boss/dead_bell/dead_bell_recover.png` | `1200x400` | 3 | `400x400` | `DEAD_BELL_RECOVER_SHEET` |
| `boss/dead_bell/dead_bell_awakened_echo_bell.png` | `720x220` | 4 | `180x220` | `DEAD_BELL_AWAKENED_ECHO_BELL_SHEET` |
...
| `boss/mirror-dream/mirror_dream_recover.png` | `1200x400` | 3 | `400x400` | `MIRROR_DREAM_RECOVER_SHEET` |
| `boss/mirror-dream/mirror_dream_awakened_cracks.png` | `1400x419` | 4 | `350x419` | `MIRROR_DREAM_AWAKENED_CRACKS_SHEET` |
| `boss/mirror-dream/mirror_dream_cast_awakened_cracks.png` | `2400x400` | 6 | `400x400` | `MIRROR_DREAM_CAST_AWAKENED_CRACKS_SHEET` |
| `boss/mirror-dream/mirror_dream_recover_awakened_cracks.png` | `1200x400` | 3 | `400x400` | `MIRROR_DREAM_RECOVER_AWAKENED_CRACKS_SHEET` |
...
| `boss/blood-moon-many-faces/blood_moon_phase_runes.png` | `1920x160` | 12 | `160x160` | `BLOOD_MOON_PHASE_RUNES_SHEET` |
| `boss/blood-moon-many-faces/blood_moon_final_stagger.png` | `1600x400` | 4 | `400x400` | `BLOOD_MOON_FINAL_STAGGER_SHEET` |
...
| `boss/blood-moon-many-faces/blood_moon_sixfold_effect.png` | `3360x350` | 8 | `420x350` | —（沿革保留，不预加载） |
| `boss/blood-moon-many-faces/blood_moon_many_faces_effect.png` | `5760x420` | 12 | `480x420` | —（沿革保留，不预加载） |

Boss 当前普通运行时轮换为 `蛛弦 -> 雾骨 -> 镜魇 -> 牙岚 -> 灯烬 -> 枯铃`，`bossKills >= 12` 后进入终幕 `万相血月`。`蛛弦` 使用 `spider-string/` 下的本体、近战攻击、蛛丝弹施法/特效、地下蛛丝柱施法/特效、千丝牢笼施法和强化蛛丝柱图集；阶段技能按 `P1 突进后近战 -> P2 解锁蛛丝弹 -> P3 解锁地下蛛丝柱` 累计保留，突进不使用独立图集：前摇按阶段化计时播放 `boss_attack.png` 前 3 帧，冲刺位移复用 `boss.png`，抵达后再从头完整播放 `boss_attack.png` 完成近战判定。`boss_pillar_cast.png` 负责向地下牵丝并上提的施法动作，多个 `boss_pillar_effect.png` 实例按落点错峰播放地下预警、上刺峰值和断丝消散，不横向平铺。`镜魇` 使用 `mirror-dream/` 下的本体、共用施法、月镜碎片、假身留影和镜中噩梦图集；P3 真身突进后按 `mirror_dream_recover.png` 的滑停、低身暴露、起身三帧覆盖 `40` 帧安全反击窗。三张 `*_awakened_cracks.png` 只包含逐帧对齐的银蓝新增裂纹，分别覆盖移动、施法与恢复本体；运行时以低透明度叠在不透明蚀醒真身上，假身不使用这些覆盖图。`牙岚` 使用 `fang-gale/` 下的本体移动、蓄力、后撤、段间转向、扑咬冲刺、终段扑咬、恢复和风牙裂波图集；`灯烬` 使用 `lantern-ember/` 下的本体、三张施法、召唤牵引、贴地火线、强化连线、蚀醒火线网格、灰烬减速区和死亡预留图集；其中火线用强化连线图集从施法者指向落点，网格每周期只在判定危险侧绘制并始终保留透明缝隙，灰烬区按完整椭圆足迹绘制。`枯铃` 使用 `dead_bell/` 下的本体、摇铃、三帧恢复、声波环和横向音刃；施法/恢复配置基值为 `228x228`，经统一 `bodyDrawScale=4/3` 后实际绘制为 `304x304`，并以 `drawBottomPadding=0` 共用贴底脚线。蚀醒时在本体后方持续绘制四帧第二裂铃：每 `9` 帧换帧，绘制 `106x130`，水平/脚线上偏移为 `68/70px`。`双调枯铃` 的停拍反震把 `dead_bell_cast.png` 前 3 帧用于 `30` 帧警告、后 3 帧用于最多 `42` 帧激活，再用 `dead_bell_recover.png` 覆盖完整 `54` 帧安全反击窗；激活期违规会立即触发反震并转入同样的 `54` 帧窗口，不留下额外激活空档。声波按每帧可见 bbox 补偿为与圆形碰撞一致的外缘；低/高频以色相、扩散速度和启动音区分。音刃前 2 帧警告，中间 2 帧覆盖飞行伤害，末 2 帧只在判定结束后消散。`mirror_shard.png` 用于可左右边界折返一次的月镜碎片，也复用于缩小、无折返的镜中噩梦弹片和技能反射弹；`mirror_afterimage.png` 用于不造成接触伤害的假身；`mirror_nightmare.png` 固定在镜影破碎位置完整播放六帧弹幕发射序列，结束后才向发射时的玩家位置散射三枚小镜片，图集本身不再作为飞行弹。`灯烬` 的蚀醒形态复用基础本体，运行时增加移动火线网格和灰烬减速区。Boss 击败仍统一即时结算并进入 `60` 帧视觉冻结：大多数 Boss 使用击杀当帧快照的随机方向裂身，雾骨先绘制三组雾图集的错相吹散，再把同一快照拆为骨架碎片散落，枯铃则以断铃和悬停声波消散完成专属收尾，万相血月按 `blood_moon_death.png` 六帧顺序剥落残相并碎裂光轮；所有路径结束后才显示奖励或胜利界面。`lantern_ember_death.png` 仍作为后续死亡状态机素材预留。万相血月的 P1-P3 使用蛛雾、镜牙、灯铃特效，P4/P5 复用六名眷属现有攻击图集，并用 `blood_moon_phase_runes.png` 的六组暗/亮符印预告来源；终相清场后以 `blood_moon_final_stagger.png` 循环覆盖完整破月输出窗口。以上 Boss 素材不在运行时目录提交 `*_source.png` 绿幕制作源图；运行时只加载透明 PNG。
...
雾骨额外使用独立的雾骨钉普攻、单点埋骨、成列压地和蚀醒合围施法图集；普攻在第 4 帧从手部高度斜向下释放独立的 4 帧骨钉飞行循环。三种地刺施法共享原有伤害与预警时序，但用不同本体轮廓提前区分单点、横排和由外向内合围。所有雾气改由 `mist_bone_fog_veil.png`、`mist_bone_fog_roll.png` 与 `mist_bone_fog_wisp.png` 三套效果图集承担：每套均为可无缝循环的 6 帧 RGBA 横向图集，运行时以不同帧节拍和相位重复铺排多组实例，用于薄雾减速区、浓雾葬遮蔽、蚀醒环身雾和击败吹散雾。雾的可见形状不再由 Canvas 椭圆或路径生成；程序只负责图集选帧、布局、透明度和既有寿命，碰撞/减速椭圆、遮蔽强度与地刺预警时序不变。蚀醒本体的冷骨光继续使用既有 Canvas `filter`，击败骨架碎片仍在雾层之后绘制。P3 成列施法结束后复用攻击动作进入锁向追击，P4 合围调用现有敌人素材生成一名蚀醒 `warden` 与一名基础召唤池同伴。
