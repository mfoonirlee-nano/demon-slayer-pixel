...
Boss 被击杀时进入 `60` 帧视觉冻结，普通玩法更新停止。大多数 Boss 会快照当帧本体，按一次性随机切向裁成两半后分离淡出；雾骨在捕获姿态后清空雾与骨刺，再把快照拆成骨架碎片散落并吹散雾团；枯铃会先冻结最多三圈当前声波（无活动声波时生成一圈回响），清空实战声波和音刃，再把独立裂铃图裁成左右两块分离；万相血月会清空六种借用判定与召唤物，不使用击杀快照，而是在同一冻结计时内推进 `blood_moon_death.png` 六帧，让残相与光轮按确定顺序剥落。装备奖励或胜利 overlay 只在效果结束后显示。这四条路径都由独立的击败效果状态推进，不恢复 Boss 战斗动作。
