...
`src/game/state.ts` 维护运行时的全局可变状态，保存玩家、敌人、Boss、平台、残灵掉落物、月潮灵匣机会/实体/开匣定格、投射物、技能效果、粒子、月亮与战斗进度等实时数据。玩家状态同时保存已收集残灵和引灵计时。`src/game/gameStore.ts` 则把运行时快照桥接到 Jotai，供 React HUD、奖励、暂停和死亡界面读取显示。奖励 overlay 的仲裁顺序是 Boss 装备、月潮灵匣、经验升级、手动暂停；三类奖励各自保留独立队列。
...
3. 地图与掉落物：未承载玩家的平台、月潮灵匣的路线光柱与平台辉光、敌人掉落的残灵。
4. 玩家技能闪屏。
5. 玩家、月潮灵匣本体、狩牙刃水刃、当前承载玩家的平台、技能效果、敌人、Boss 本体动作、Boss 击杀裂身快照、Boss 技能及其序列帧预警、投射物、粒子；屏外宝藏方向标记最后覆盖在世界层上。
6. 地面瓦片覆盖层。
7. 可选碰撞区域调试层。
