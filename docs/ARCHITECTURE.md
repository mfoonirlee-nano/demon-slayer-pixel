...
Boss 被击杀时会快照其当帧本体图像并进入 `60` 帧视觉冻结：普通玩法更新停止，大多数 Boss 继续把快照按一次性随机切向裁成两半后分离淡出；雾骨改走同一冻结协议内的专属分支，把快照拆成骨架碎片散落并吹散雾团；枯铃则在同一裂身协议前冻结最多三圈当前声波（无活动声波时生成一圈回响），并把独立裂铃图裁成左右两块分离，保证铃身明确断开。装备奖励或胜利 overlay 只在效果结束后显示。三条路径都拥有独立计时，不修改 Boss 动作状态或预留死亡序列。
