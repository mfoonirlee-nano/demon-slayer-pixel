...
- `src/entities/`:
  ...
  - `bosses/attackTelegraphs.ts`: 从 Boss 当前动作和真实释放计时解析统一攻击预警，并绘制蓄力环、锁向带与传送落点；不参与伤害结算。
...
4. 玩家技能闪屏。
5. 玩家、狩牙刃水刃、当前承载玩家的平台、技能效果、敌人、Boss、Boss 击杀裂身快照、Boss 技能、投射物、粒子。
6. 地面瓦片覆盖层。
7. Boss 攻击预警覆盖层。
8. 可选碰撞区域调试层。

Boss 攻击预警不维护第二套攻击倒计时。`attackTelegraphs.ts` 直接读取各招式已有的 `actionTimer`、`castTimer`、释放帧和阶段化冲刺前摇，生成纯视觉 cue；技能实体生成后，地刺、牢笼、火线、音刃等精确落点预警仍由各自 effect 模块接管。统一预警层在战斗粒子之后、碰撞调试之前绘制，因此蓄力和锁向信息不会被角色或技能特效完全遮住，同时也不会被误记为有效伤害区域。

碰撞调试层在真实判定发生处记录当帧几何，因此能够保留命中后立即移除的投射物、一次性冲击盒和扫掠盒。它覆盖实体受击盒、地面与平台单向落脚面、拾取物、玩家与敌方攻击，以及矩形、椭圆、圆环和分段危险带；粒子、残影、拖尾等不参与空间判定的纯视觉对象不会被记录。
