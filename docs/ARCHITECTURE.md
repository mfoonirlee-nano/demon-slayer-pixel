...
2. 近景背景：树线、当前幕出生遮挡物、Boss 地标、石塔、鸟居。
3. 地图与奖励：未承载玩家的平台、水晶、宝箱。
4. 玩家技能闪屏。
5. 玩家、当前承载玩家的平台、技能效果、敌人、Boss、Boss 技能、投射物、粒子。
6. 地面瓦片覆盖层。
7. 可选碰撞盒调试层。

平台层级由 `player.onPlatform` 动态决定：玩家落台前，平台完整绘制在玩家下层；成功落台后，只有当前承载平台会移到玩家上层；玩家离台时恢复到下层。

逐幕出生遮挡物通常绘制在实体下层；普通敌人选中遮挡出生后，`drawEnemy()` 会在该敌人的完整视觉边界内，按相同近景速度裁剪重绘物件，并沿行进方向逐帧收窄遮挡，使敌人从树干、根部或岩体后显露而不覆盖附近玩家。

React HUD 不直接绘制到 Canvas，而是订阅 `gameSnapshotAtom` 显示生命、技能资源、大招能量、Boss 血条、暂停和死亡状态。角色状态 HUD 由左侧大招圆框、贴在大招右下方的当前技能圆框、以及两条随等级增长的生命/技能资源条组成。
