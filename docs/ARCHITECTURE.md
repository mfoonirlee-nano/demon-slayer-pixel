...
Boss 读招不维护第二套攻击倒计时，也不使用独立的程序化红色虚线绘制层。`renderBoss.ts` 直接读取已有的 `actionTimer`、`castTimer` 和反震计时推进本体序列：常规施法使用各招对应的 cast 图集，蛛弦突进前摇使用 `boss_attack.png` 前 3 帧，雾骨 P3 成列施法结束后沿施法时锁定的朝向追击，牙岚按后撤、段间转向、终段扑咬和恢复状态切换专用本体图集，枯铃停拍反震则把 `dead_bell_cast.png` 前 3 帧映射到警告窗口、后 3 帧映射到激活窗口。技能实体生成并锁定危险位置后，蛛丝柱、牢笼、地刺、风牙裂波、火线、声波、音刃和终幕技能等由各自 effect 模块在原有警告时序内播放特效图集的早期帧，再继续推进命中与消散帧；雾骨浓雾葬先绘制局部 Canvas 雾层，再绘制由外向内错峰激活的骨刺预警。这些视觉帧不复制或改变伤害计时。
